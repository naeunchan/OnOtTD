...
import { WeatherMode, WeatherModeOverride, resolveWeatherMode } from '../weather/weatherModeResolver';
...
export function SandboxWeatherModeCard({ value, onChange }: SandboxWeatherModeCardProps) {
  const effectiveMode = resolveWeatherMode(value, buildTimeWeatherMode);

  return (
    <Section
      title="샌드박스 날씨 모드"
      description="실기기 QA에서 실시간 경로와 mock 폴백 UI를 강제로 재현할 수 있습니다."
    >
      ...
      <Text style={styles.summary}>{getModeSummary(value, effectiveMode)}</Text>
    </Section>
  );
}
